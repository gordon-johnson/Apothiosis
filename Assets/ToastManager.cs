...
public class ToastManager : MonoBehaviour
{
    ...
    public bool skipCurrentToast = false;

    public static float timeScale = 0.0001f;
    ...
    void Awake()
    {
        ...
        visible_pos = new Vector3(0, -200, 0);
    }
    ...
    {
        // enques for 6 hours
        instance.requests.Enqueue(new ToastRequest(msg, 21600));
    }
    ...
    void Update()
    {
        ...
        if (!toasting && requests.Count > 0)
        {
            ...
            float toastDuration = instance.show_duration;
            if (new_request.duration > 0) toastDuration = new_request.duration;
            instance.StartCoroutine(DoToast(instance.ease_duration, show_duration));
        }
    }

    static IEnumerator DoToast(float duration_ease_sec, float duration_show_sec)
    {
        instance.skipCurrentToast = false;
        ...
        float waitTime = duration_show_sec;
       // Debug.Log("Wait time is: " + waitTime.ToString());
        float showProgress = 0;
        // re writing this so that it can be skipped out of
        // fixme this makes no fucking sense
        // when I do showProgress < .0005 then delta time ~= .017, when I do showProgress < 5 then deltatime ~= 1.7 * 10^-6
        while (showProgress < waitTime)
        {
            showProgress += Time.deltaTime;
            //Debug.Log("Delta time is: " + Time.deltaTime.ToString());
            if (instance.skipCurrentToast)
            {
                instance.skipCurrentToast = false;
                break;
            }
...
public class ToastRequest
{
    ...
    public ToastRequest(string s, float inDuration = -1)
    {
        message = s;
        duration = inDuration;
    }
